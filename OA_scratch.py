...
for i, name in enumerate(edi_names):
    ...
    # print(f'\nnew_Z_obj.z_err[0,0,0] = {new_Z_obj.z_err[0,0,0]}\n')
    # print(f'\npossible value = {abs(np.real(new_Z_obj.z[0,0,0]))*0.2}\n')
    # print(f'\nassigned value = {df.loc[(df["Code"]==sites[i]) & (df["Component"]=="ZXX"), "Error"].iloc[0]}\n')



df.to_csv(path_or_buf=savepath+r'/modem.data', sep=' ', header=False, index=False)
data_header = "# written by a dog's breakfast of mtpy (Kirkby et al.) and pandas assembled Oliver Azevedo, plus M3D from University of Alberta, 2018. Based on M3DET by Ersan Turkoglu (2006).\n# Period(s) Code GG_Lat GG_Lon X(m) Y(m) Z(m) Component Real Imag Error\n> Full_Impedance\n> exp(+i\omega t)\n> [mV/km]/[nT]\n> 0.00\n> 10.0836 -85.3647\n> 35 36\n"
with open(savepath+r'/modem.data', 'r') as original: data = original.read()
with open(savepath+r'/modem.data', 'w') as modified: modified.write(data_header + data)
